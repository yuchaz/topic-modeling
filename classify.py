...
def main():
    ...
    with open(os.path.join(LDA_DIR, 'text_corpus.classify'), 'r+') as decif:
        categories = np.array(decif.read().split('\n'))
    decif.close()
    ...
    sparse_matrix = gensim.matutils.corpus2csc(corpus).transpose()
    clf = SVC()
    clf.fit(sparse_matrix, categories)
